...
// Selector to ignore  -- -NOT BEING USED BECASUE SLOW
        ...
        if ( node ) {
            ...
            while (node) {
                ...
                if ( node.nodeType === 3 ) {
                    ...
                    val = node.nodeValue;
                    pnn = node.parentNode.name
                    if (pnn !== 'a' && pnn !='h1' && pnn !== 'h2' && pnn !== 'h3' && pnn !== 'h4' && pnn !== 'script')
                    {
//                        console.log("nnn", node.parentNode.name, val)
                        // The new value.
...
function annotateText(entries, glossaryFilePath, $) {
    ...
    console.log("Modifier: AnnotateText() ", new Date(), "(glossary)");
    ...
    entries.forEach(function(entry) {
        ...
        $('*').each(function() {
            ...
            // This was super slow so instead I do the checks in replaceText() - roughly 3x faster now
            // if (
